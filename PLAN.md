...
- **Tempo/OTel** — SDK sets resource attributes `app`, `service`, `env`, `host` with those flat names, *plus* `service.name = "{app}-{service}"` and `deployment.environment.name` for semconv/service-graph compatibility. Tempo span-metrics `dimensions: [app, service, env, http.route, http.response.status_code]`.
...
- **Tempo**: `compactor.compaction.block_retention: 168h` (7d); span-metrics `dimensions: [app, service, env, http.route, http.response.status_code]`; **keep `local-blocks` disabled** — the reference documents a 12 GB memory regression from it. Record as do-not-re-enable in `docs/operations.md`.
