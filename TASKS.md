...
- [x] Deploy to the VPS — LGTM live at `grafana.obs.atick.dev` / `ingest.obs.atick.dev` behind Dokploy's Traefik (`OBS_EDGE_NETWORK=dokploy-network`, `OBS_EDGE_EXTERNAL=true`), Let's Encrypt chain valid, all three ingest paths 401 without credentials and a non-ingest path 404s. No Dokploy domain entry is needed for either host: its wizard maps one host to one service and one port, which cannot express `ingest.<domain>`, and the labels in `compose.yml` already do the whole job. GlitchTip on Dokploy is still open — it needs a merged compose file or a mount for `.env.glitchtip`, which is `required: true` and gitignored.
