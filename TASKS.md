...
- [x] **Push.** The repo exists and `origin` is set; the first push was the user's to run — `git push` is denied by this environment's settings, and routing around that with `gh repo create --push` would defeat a guardrail that is there on purpose. Done at `2d4bcf2`.
- [x] Deploy to the VPS — LGTM live at `grafana.obs.atick.dev` / `ingest.obs.atick.dev` behind Dokploy's Traefik (`OBS_EDGE_NETWORK=dokploy-network`, `OBS_EDGE_EXTERNAL=true`), Let's Encrypt chain valid, all three ingest paths 401 without credentials and a non-ingest path 404s. No Dokploy domain entry is needed for either host: its wizard maps one host to one service and one port, which cannot express `ingest.<domain>`, and the labels in `compose.yml` already do the whole job.
- [ ] Deploy GlitchTip to the VPS — repo side is done; creating the Dokploy service is the user's step. A second Compose service pointing at `compose.glitchtip.yml`, at `errors.obs.atick.dev` (DNS already resolves). Two things had to change first, both of which only ever failed on a deploy and never locally. The file did not render on its own: `glitchtip-web` joins the `obs` and `edge` networks, which only `compose.yml` declared, so a platform that deploys one compose file per service got `refers to undefined network obs`. And `.env.glitchtip` was `env_file:` with `required: true` while being gitignored, so a platform that re-clones the repo never had it. Both networks are now declared in `compose.glitchtip.yml` as well — identically, so the overlay still merges — and `make glitchtip-config-check` renders the standalone shape as the regression guard, mirroring `make config-check`.
- [x] Tag `v0.1.0` — `cd.yml` green end to end on the first run.
- [ ] Onboard the first real app
...
- [x] Publish `obstack` to PyPI — **live at 0.1.0**. Trusted Publishing worked on the first attempt, so none of the three things that must agree (repository, `cd.yml`, environment `pypi`) was misconfigured. Verified as a consumer rather than from the workflow log: `uv pip install --no-cache obstack` into an empty venv resolves 0.1.0 from the index, pulls its six runtime dependencies, and imports. `uv add obstack` in `docs/onboarding-an-app.md` is now a real instruction rather than a git URL.
...
- [x] **`verify-dashboards.sh` substituted the template variables away, so it verified 51 panels and zero variables.** Found on the live VPS: the Infrastructure dashboard's Containers row rendered empty while the Host row above it worked. The data was never the problem — cAdvisor was emitting 16 series, `remote_write` reported `samples_failed_total` 0 / `dropped` 0 / `out_of_order` 0 across 24.4M samples, and on the VPS `count(container_memory_working_set_bytes{app="demo",env="local",host="demo-host",name!=""})` returned **15**. Grafana's own datasource proxy returned the full container list for the exact request the variable issues. The option list was correct throughout; what was empty was the *selection*, most likely a stale `?var-container=` URL parameter, which Grafana persists and which survives a page reload — matching the two things that did not fix it (reloading, widening the range) and the one that did (arriving fresh). Two design choices turned that transient state into something permanent-looking and undiagnosable: `includeAll: true` with `allValue: null` makes "All" over an empty option list interpolate to `""`, so `name=~""` matches nothing and looks exactly like a working dashboard; and `refresh: 1` recomputes options only on dashboard load, so it never self-corrects. All three dashboards had the same latent defect on their leaf variable (`$container`, `$service`, `$datname`) — now `allValue: ".+"`, which is exactly the union of the options because Prometheus never returns an empty label value, and `refresh: 2` throughout. **The real finding is why this shipped**: `substitute()` replaced `$container` with `.*` and `$service` with `api|loadgen` before asserting the panels returned rows, so what CI checked was the panel expression with values the script invented, and no variable query was ever executed. The script now resolves each dashboard's `templating.list[]` against Prometheus in dependency order and feeds the results into `substitute()`, so `$datname` is checked as `demo|postgres|template0|template1` rather than the hardcoded `demo`. Both new checks were proven to fail by breaking them. The general lesson: a test that stubs out the thing under test passes hardest exactly when that thing is broken.
- [x] **`env_file:` and "deployable from git" are incompatible, and GlitchTip was designed around the wrong one.** `.env.glitchtip` was a separate file *because* `env_file:` hands values to the container verbatim — a `$` is a `$`, with none of the `$$`-doubling `.env.server` needs — and that reasoning was written into three places. It is sound, and it is unreachable on any platform that re-clones the repo on every deploy: the file is gitignored, so it is never there, and a platform's environment UI writes a `.env` that Compose reads for *interpolation*, which cannot satisfy an `env_file:`. So the choice was between the literal-`$` property and being deployable the same way everything else here is. Deployability won: `env_file:` is gone, every setting is now `environment:` interpolated from `.env.glitchtip` (passed as `--env-file`, the same mechanism the Makefile already uses for `agent/.env.agent`) or from the platform's UI. The cost is real and is now documented in all three places rather than left contradicting the code — the one value it can bite is `EMAIL_URL`, whose SMTP password is not ours to generate. What was *not* done is the half-measure: leaving `env_file:` alongside `environment:` would have kept the file present and inert, because Compose ranks `environment:` higher — precisely how `agent/.env.agent` looked authoritative while doing nothing (`9d33693`). Verified the swap is transparent by comparing every variable inside the running container against its source and re-running `verify-errors.sh` unaltered: 5/5, including the derived `DATABASE_URL` connecting to the pre-existing `glitchtip_pg_data` volume. Two smaller things fell out of the same read: `DATABASE_URL` was a fourth copy of a credential the other three variables already carry, so it is now assembled from them; and `.env.glitchtip.example` shipped `ENABLE_USER_REGISTRATION=false` while its own comment said to leave it `true` for the first signup, which on a fresh deploy means no way to create the first account short of `manage.py createsuperuser` on the box.
- [ ] **`scripts/verify-errors.sh` cannot verify a deployed GlitchTip.** It reads through `docker compose exec` and `localhost:8001` — deliberately, since the REST API needs a token that only exists after someone has logged into the UI, and a script that cannot run until a human has clicked through a web app is not a verification script. That reasoning still holds, but it means the deployed instance has no automated check; it is verified by hand, or by running the script on the VPS. Same shape as `scripts/backup.sh`, which reaches GlitchTip's database the same way.
